...
const registerSchema = z
  .object({
    firstName: z.string().min(2, "At least 2 characters").max(100).trim(),
    lastName: z.string().min(2, "At least 2 characters").max(100).trim(),
...
export default function RegisterPage() {
  ...
  return (
    <AuthShell
      ...
      subtitle="Password registration sends a verification code. OAuth sign-in verifies email with your provider."
      footer={
    ...
    >
      {(hasGoogle || hasGithub) && (
        <div className="mb-8 space-y-3">
          ...
          <div className="relative py-2 text-center text-xs text-[var(--text-subtle)]">
            <span className="bg-[var(--surface-0)] px-2">or register with email</span>
      ...
      {errorMessage && (
        <div className="mb-6 rounded-[var(--radius-md)] border border-[var(--danger)]/40 bg-[var(--danger-soft)] px-3 py-2 text-center text-sm text-[var(--danger)]">
          {errorMessage}
        </div>
      )}
      ...
      >
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            />
            {errors.firstName && (
              <p className="mt-1 text-sm text-[var(--danger)]">{errors.firstName.message}</p>
            )}
          </div>
          <div>
            <label className="mb-1 block text-xs font-medium text-[var(--text-muted)]">
            ...
            />
            {errors.lastName && (
              <p className="mt-1 text-sm text-[var(--danger)]">{errors.lastName.message}</p>
            )}
          </div>
        </div>
        <div>
          ...
          <input type="email" autoComplete="email" className={inputCn()} {...register("email")} />
          {errors.email && (
        ...
        <button type="submit" disabled={mutation.isPending || oauthBusy !== null} className={btnPrimaryCn("w-full")}>
