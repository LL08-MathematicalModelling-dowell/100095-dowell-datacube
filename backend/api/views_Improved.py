...
#         """
#         Handles the POST request to create a new database with specified collections.
#         This method performs the following steps:
#         1. Validates the request data using `AddDatabasePOSTSerializer`.
#         2. Extracts the database name and collections from the validated data.
#         3. Ensures that at least one collection with fields is specified.
#         4. Checks if the database already exists.
#         5. Starts an atomic transaction to create the database and collections.
#         6. Inserts metadata and initializes the database.
#         7. Commits the transaction if successful, or aborts it and rolls back changes on failure.
#         Args:
#             request (Request): The HTTP request object containing the data for creating the database.
#         Returns:
#             Response: A Response object with the result of the database creation process.
#                 - On success: Returns a 201 status with a success message and details of the created database and collections.
#                 - On failure: Returns a 400 status if validation fails or the database already exists, or a 500 status for other errors.
#         """
...
from .api_auth_utils import api_key_required, append_user_id, remove_user_id, process_database_metadata
...
class CreateDatabaseView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            # Append user ID to database name
            db_name_with_uid = append_user_id(db_name, request.user_id)
            ...
            # Check if the database already exists
            if self.check_database_exists(db_name_with_uid):
                return Response(
                    {"success": False, "message": f"Database '{db_name}' already exists!"},
                    status=status.HTTP_400_BAD_REQUEST
                )
            ...
            try:
                # Insert metadata and initialize the database
                db_metadata = self.insert_metadata(db_name_with_uid, collections, session)
                collection_metadata = asyncio.run(self.create_collections_in_db(db_metadata, collections, session))

                # Commit the transaction
                session.commit_transaction()

                # Remove user ID from response data
                response_data = {
                    "success": True,
                    "message": f"Database '{db_name}' and collections created successfully.",
                    "database": {
                        "name": remove_user_id(db_metadata.get("database_name")),
                        "id": str(db_metadata.get("_id"))
                    },
                    "collections": collection_metadata
                }
                ...
                session.abort_transaction()
                logger.error(f"Transaction aborted due to error: {inner_exception}", exc_info=True)
                self.rollback_collections(cluster[db_name_with_uid], [coll['name'] for coll in collections])
                raise inner_exception
