...
def log_db_operation_task(self, data: dict):
    try:
        ...
        logger.error(f"Failed to log DB operation: {e}")
...
def aggregate_daily_usage():
    ...
    pipeline = [
        ...
        {"$group": {
            ...
            "top_collections": {"$addToSet": "$collection"}  # simplistic
        }},
        {"$project": {
            "user_id": "$_id",
            "date": date_str,
            ...
            "top_collections": {"$slice": ["$top_collections", 10]},
        }}
    ]
    results = svc.db["http_requests"].aggregate(pipeline)
    for doc in results:
        svc.db["daily_aggregates"].update_one(
            {"user_id": doc["user_id"], "date": date_str},
            {"$set": doc},
            upsert=True
        )
    logger.info(f"Daily aggregation for {date_str} completed")
...
def cleanup_old_analytics(days_to_keep=30):
    ...
    svc = AnalyticsService()
    collections = ["http_requests", "db_operations", "performance_metrics",
                   "client_info", "errors", "mongo_details", "slow_queries"]
    for coll in collections:
        result = svc.db[coll].delete_many({"timestamp": {"$lt": cutoff}})
        logger.info(f"Deleted {result.deleted_count} from {coll}")
